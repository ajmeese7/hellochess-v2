...
export default class Connection {
    ...
    constructor(private io) {
        this.players = [];
        this.rooms = [];
        // Creates Global room on startup
    ...
    createNewRoom(roomName: string, gameType: string, time: any, roomObj: any, host: any): Room {
        ...
        if (roomObj.roomMode === "match") {
            ...
            // Checks to see if the invitedPlayer is online
            if( invitedPlayer && invitedPlayer.socket &&
                challenger && challenger.username) {
                // Sends the invitedPlayer an invitation to join the room
    ...
    duplicateUser(playerId: string, ipaddress) {
        ...
        if (process.env.NODE_ENV !== "production") {
            // Allows duplicate logins on dev servers for testing purposes
            return false;
        }
    ...
    updatePlayer(data) {
        // Checks to see if the player is in the player list
    ...
    // Removes a player by their socket obj
