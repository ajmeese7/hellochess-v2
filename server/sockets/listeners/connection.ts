...
module.exports = function(io, socket, connection) {
    ...
    function disconnect() {
        // Retrieves the user that has disconnected
        ...
        // Gets a list of rooms that the player is in
        ...
        rooms.map((room: Room) => {
            ...
            if (room.empty() && room.name !== 'Global') { // There are no users in the room
                ...
                // Tells everyone that the player has left the room
    ...
    socket.on('connected-user', data => {
        ...
        // Checks to see if the player is already connected elsewhere
