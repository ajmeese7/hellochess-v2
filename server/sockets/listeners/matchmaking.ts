...
module.exports = function(io, socket, connection) {
    ...
    // The user wants to stop being paired
    socket.on('stop-pairing', () => {
        // Get the player that is requesting to be paired
    ...
    socket.on('pair-me', (data) => {
        ...
        // Removes any prior instances of this player from the queue
        connection.removePlayerFromQueue(player);

        // Gets the current Queue
        let queue = connection.queue;

        // Searches for an opponent (FIFO)
