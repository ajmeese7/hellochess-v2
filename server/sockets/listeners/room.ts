...
module.exports = function(io, socket, connection) {
    ...
    socket.on('new-message', data => {
        ...
        // Tells everyone there is a new message
    ...
    socket.on('sit-down-board', data => {
        ...
        if (room.gameReady()) {
            // Starts the game if all players are seated
    ...
    socket.on('leave-room', data => {
        ...
        // Retrieves the user and room which are being updated
        let room: Room = connection.getRoomByName(roomName);
        let player: Player = connection.getPlayerBySocket(socket);
        ...
        if (room.empty() && roomName !== 'Global') { //there are no users in the room
            if (!connection.removeRoomByName(roomName)) {
                // TODO: not sure if anything is needed in an else
    ...
    socket.on('create-room', data => {
        ...
        if (!data.gameType || !player || room.isPlayerInRoom(player) == true) {
            // TODO: Error
            return;
        }
        ...
        if (room.voiceChat && room.getNumberOfPlayers() == 10) {
            ...
            // Tells the player that this is a voice chat room and
            // that there are too many players
        ...
        // Removes previous instances of this player
    ...
    socket.on('join-room', data => {
        ...
        // TODO: !data.gametype condition as in above function?
        let player: Player = connection.getPlayerBySocket(socket);
        if (!player || room.isPlayerInRoom(player) == true) {
            return;
        }
    ...
    socket.on('resign', data => {
        ...
        // Adds resign message to the room
    ...
    socket.on('rematch-offer', data => {
        ...
        if (room.rematchOffered === true) {
            // If the other player offered a rematch already, start the game
